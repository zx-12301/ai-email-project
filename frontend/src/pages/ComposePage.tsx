...
import {
  ...
  List, ListOrdered, Link2, Minus, X, Sparkles, Users, Search, User
} from 'lucide-react'
...
export default function ComposePage() {
  ...
  useEffect(() => {
    const initPage = async () => {
      try {
        ...
        // 加载系统内所有用户
        try {
          const usersData = await authApi.getAllUsers(true)
        ...
        if (draftId) {
          try {
            // 加载草稿数据
            const draftData = await mailApi.getMailById(draftId)
            setFormData({
  ...
  const addRecipient = (email: string) => {
    const newTo = formData.to ? `${formData.to}, ${email}` : email
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const draftId = params.get('draft')
      if (draftId) {
        await mailApi.deleteMail(draftId)
      }

      showToast(`邮件发送成功！${sendViaSmtp ? '已通过 QQ 邮箱发送' : '已保存到已发送文件夹'}`, 'success')
      navigate('/inbox')
    } catch (error: any) {
          ...
          <div className="max-w-5xl mx-auto p-6 space-y-4">
            ...
            <div className="flex items-start gap-3 border-b border-slate-200 pb-4">
              ...
              <div className="flex-1 relative">
                ...
                {/* 联系人选择器 */}
                {showContactPicker && (
                  <div className="absolute top-full left-0 mt-2 w-96 bg-white border border-slate-200 rounded-lg shadow-lg z-20 max-h-96 overflow-hidden">
                    {/* 搜索框 */}
                    ...
                    <div className="overflow-y-auto max-h-80">
                      ...
                      {filteredContacts.length > 0 && (
                        <div>
                          <div className="px-3 py-2 bg-slate-50 text-xs font-medium text-slate-500 flex items-center gap-1.5 sticky top-0">
                            <User className="w-3.5 h-3.5" />
                            我的联系人 ({filteredContacts.length})
                          </div>
                          {filteredContacts.map((contact) => (
                            <div
                              key={contact.id}
                              onClick={() => addRecipient(contact.email)}
                              className="flex items-center gap-3 px-3 py-2.5 hover:bg-blue-50 cursor-pointer border-b border-slate-100"
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="text-sm font-medium text-slate-900 truncate">{contact.name || '未命名'}</div>
                      ...
                      {filteredUsers.length > 0 && (
                        <div>
                          <div className="px-3 py-2 bg-slate-50 text-xs font-medium text-slate-500 flex items-center gap-1.5 sticky top-0">
                            ...
                            系统用户 ({filteredUsers.length})
                          </div>
                          {filteredUsers.map((user) => (
                            <div
                              key={user.id}
                              onClick={() => addRecipient(user.email || `${user.phone}@example.com`)}
                              className="flex items-center gap-3 px-3 py-2.5 hover:bg-blue-50 cursor-pointer border-b border-slate-100"
                            >
                              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center text-white text-sm font-medium flex-shrink-0">
                                {(user.name || user.phone).charAt(0).toUpperCase()}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="text-sm font-medium text-slate-900 truncate">{user.name || '用户'}</div>
