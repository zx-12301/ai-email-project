...
export default function SpamPage() {
  ...
  const loadSpamEmails = async () => {
    try {
      ...
      const allEmails = currentPage === 1 ? [...mockSpamEmails, ...transformedEmails] : transformedEmails;
      setSpamEmails(allEmails);
      setTotalEmails(result.total ? result.total + (currentPage === 1 ? mockSpamEmails.length : 0) : allEmails.length);
    } catch (error) {
      console.error('加载垃圾邮件失败:', error);
      // API 失败时显示测试数据
      if (currentPage === 1) {
        setSpamEmails(mockSpamEmails);
        setTotalEmails(mockSpamEmails.length);
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleEmailClick = (emailId: number | string, isTest?: boolean) => {
    ...
    navigate(`/mail/${emailId}`, { state: { from: '/spam' } });
  };
